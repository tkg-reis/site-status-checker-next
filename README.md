...
- ~~DBのスキーマにかかる型の自動出力をしておく。そして型定義をコード上でし直してみる。~~
- ~~RLSの理解、適切な設定~~
  - ~~defaultでauth.uidを設定するようにしたほうがいいかも(インサート処理が通らないから)~~
    - 参考
      - CREATE POLICY "Allow authenticated users to insert own data"
        ON your_table_name FOR INSERT
        TO authenticated
        WITH CHECK (created_by IS NULL OR created_by = auth.uid()); -- NULLまたは自分自身の場合のみ許可
      - ALTER TABLE your_table_name ALTER COLUMN created_by SET DEFAULT auth.uid();　＝＞　こちらでいけるかも、上記は非推奨
- 登録したURLデータの参照更新削除及びユーザー情報参照が正しい取得方法、設計で行われているか確認する
  - 例えば今の設計だとブラウザ側からもinsertなどの処理がはいっているか。それともサーバーで入っているのか。またログインしたときにURLリクエスト、リロードしたときにURLリクエストするときにどうデータをinsertしていくかなど、要はサーバーコンポーネントとクライアントコンポーネントを正しく使い分けできているか確認する。
- ~~ログインしたユーザー情報に基づいたデータが来るように実装を追加する。またログインしているユーザーの情報が間違いなく表示されるようにする。クッキー、セッション残り、コンポーネントキャッシュを見直す~~
- ~~テーブルにuser情報の追加とユーザーが削除された時のフォーリンキーの動作確認~~
- ~~新しく作成したプロジェクトの疎通確認~~
- 本番用(prod)用のDBスキーマも導入する？
- 今一度要件を確認して漏れがないかチェックして必要があれば実装を加える。　＝＞　中
  - ~~/topに遷移したときまたはその画面でリロードしたときに登録してあるデータから今の監視対象のステータスコードなどを取得描画する処理の追加~~
- next buildを走らせて動作確認する。
- 脆弱性の確認
  - node.jsのパッチをいれる。
----
- gitのコミットはなるべく分散させる。ジョブ関連の実装は切り戻し処理しておく
- 今回目指すのはログイン機能、CRUD操作のみとしクーロン実装はしない（ログイン実装に時間がかかるようならそれはおいておいてテストをやる。※要件の優先順位としてログイン機能が低いため（ログインが可能で余裕があればplaywrightでテストしてみる、できなければ軽微でもいいのでテストをしておく）
- そのほかはメモをチェックする
